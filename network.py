...
class Multimodal_Transformer(nn.Module):
    def __init__(self, ntoken=128, ninp=50, nhead=5, nhid=512, dropout=0.0, nlayers=3):
        super(Multimodal_Transformer, self).__init__()
        from torch.nn import TransformerEncoder, TransformerEncoderLayer

        self.ntoken = ntoken
        self.ninp = ninp
        self.linear1 = nn.Linear(1,ntoken)
    ...
    def forward(self, input1, input2): #[bs,50,1] , [bs,50,1,1]
        ...
        li1 = self.linear1(input1)

        #- linear [bs,50,1,1] -> [bs,50,D]
        input2 = input2.squeeze(3)
        li2 = self.linear1(input2)

        li = torch.cat([li1, li2], dim=2) # [bs,50,2D]
        
        #- transformer [bs,50,2D] -> [bs,50,2D]
        tf = li.permute(2,0,1) #[2D,bs,50]
        tf = self.transformer_encoder(tf)
        tf = tf.permute(1,2,0)

        #- parse cls token [bs,50,D] -> [bs,50,1]
        cls_token1 = tf[:,:,0].unsqueeze(2)
        cls_token2 = tf[:,:,self.ntoken].unsqueeze(2)
