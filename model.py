...
class Generator(nn.Module):
    ...
    def forward(self, x_sig, x_freq):
        # unimodal encoder
        latent_signal = self.signal_encoder(x_sig)  #[bs,50,1]
...
class BeatGAN(AD_MODEL):
    ...
    def train_epoch(self):
        ...
        for data in self.dataloader["train"]:
            ...
            if (epoch_iter  % self.opt.print_freq) == 0:
                print("Epoch: [%d] [%4d/%4d] D_loss(s/f): %.6f/%.6f, G_loss(s/f): %.6f/%.6f" %
    ...
    def get_generated_x(self):
        fake = self.G(self.fixed_input_s, self.fixed_input_f)[0]
    ...
    def validate(self):
        ...
        :return: auc
        '''
        y_s, y_pred_s, y_f, y_pred_f=self.predict(self.dataloader["val"])
